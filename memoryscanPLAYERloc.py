...
from tools import select_endless_pid

# Requirement: XY above 4
...
def is_pattern_match(buffer, offset, debug=False):
    ...
    try:
        ...
        if buffer[offset + 1] != 0 or buffer[offset + 2] != 0 or buffer[offset + 3] != 0:
            if debug and (0x04F04BB0 <= addr <= 0x04F04BE0):
                print(
                    f"  Fail at pos 1-3: Values {buffer[offset + 1]:02X} {buffer[offset + 2]:02X} {buffer[offset + 3]:02X} should be zeros")
            return False
        ...
        if buffer[offset + 5] != 0 or buffer[offset + 6] != 0 or buffer[offset + 7] != 0:
            if debug and (0x04F04BB0 <= addr <= 0x04F04BE0):
                print(
                    f"  Fail at pos 5-7: Values {buffer[offset + 5]:02X} {buffer[offset + 6]:02X} {buffer[offset + 7]:02X} should be zeros")
            return False
        ...
        if buffer[offset + 10] != 0 or buffer[offset + 11] != 0:
            if debug and (0x04F04BB0 <= addr <= 0x04F04BE0):
                print(
                    f"  Fail at pos 10-11: Values {buffer[offset + 10]:02X} {buffer[offset + 11]:02X} should be zeros")
            return False
        ...
        if buffer[offset + 14] != 0 or buffer[offset + 15] != 0:
            if debug and (0x04F04BB0 <= addr <= 0x04F04BE0):
                print(
                    f"  Fail at pos 14-15: Values {buffer[offset + 14]:02X} {buffer[offset + 15]:02X} should be zeros")
            return False
...
def scan_memory_chunk(pm, start_addr, chunk_size, scan_number, debug_mode=False):
    ...
    try:
        ...
        contains_example = (0x04F04BB0 <= start_addr <= 0x04F04BE0) or (
                    start_addr <= 0x04F04BB0 <= start_addr + chunk_size)
...
def run(pid):
    ...
    try:
        ...
        print(
            f"Pattern: (digit 4-180) 00 00 00 (digit 4-180) 00 00 00 ?? ?? 00 00 ?? ?? 00 00 00 00 00 00 00 00 00 00 ?? ?? FF FF ?? ?? FF FF")
