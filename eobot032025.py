...
def initializeAddresses():
    global MOB_BASE_ADDR
    global CHAR_X_ADDR
    global CHAR_Y_ADDR
    global FACE_ADDR
    global Y_ADDR
    global X_ADDR
    global SPAWN_FACE_ADDR
    global SPAWN_Y_ADDR
    global SPAWN_X_ADDR
    global MOB_ID_ADDR1
    global MOB_ID_ADDR2
...
def press_ctrl_for_interaction(pm):
    ...
    try:
        ...
        # Alternate detection for changing non-zero values
            ...
            print(
                f"Kill detected! Values changed: {before_kill_val1}->{after_kill_val1}, {before_kill_val2}->{after_kill_val2}")
            kill_detected = True
            hit_detected = True
    except Exception as e:
...
def move_toward_mob(pm, mob_coords, char_x, char_y, mob_id=None, tracked_mobs=None, targeting_locked=False):
    ...
    if x_diff > y_diff:
        # Horizontal first
        if mob_x > char_x:
            primary_moves.append('right')
        elif mob_x < char_x:
            primary_moves.append('left')

        # Then vertical
        if mob_y > char_y:
            primary_moves.append('down')
        elif mob_y < char_y:
            primary_moves.append('up')
    else:
        # Vertical first
        if mob_y > char_y:
            primary_moves.append('down')
...
def run(pid):
    ...
    print(
        f"Starting with movement: {INITIAL_MOVEMENT_DURATION * 1000:.0f}ms, Ctrl: {INITIAL_CTRL_DURATION * 1000:.0f}ms")
