        ...
        while total_scan_count < scan_limit and (total_scan_count < MIN_SCANS or not valid_addresses):
            ...
            if scan_results:
                if DEBUG:
                    ...
                    if total_scan_count == 1 or total_scan_count % 10 == 0 or total_scan_count == scan_limit:
                        print(f"Scan #{total_scan_count}: Found {len(scan_results)} matching patterns.")
                ...
                printd(f"Scan #{total_scan_count}: No matching patterns found.")

            # Wait for next scan
            if not valid_addresses or total_scan_count < MIN_SCANS:
                printd(f"Waiting for next scan (1 second)...")
                time.sleep(1)
